...
(function(){
    var
        monthNames = ag.u.date.getMonthNames(),
        weekdayNames = ag.u.date.getWeekdayNamesShort(),
        dayOffset = ag.u.date.getFirstDayOfWeek(),
        ...
        {
            var $table = ag.u.tpl("agitui-calendar", "table");
        ...
        fillHeader = function($table, calInst)
        {
            ...
            $table.find("thead .month .name").text(ag.u.sprintf("%s %s", monthNames[calInst.month - 1], calInst.year));
        ...
        renderDay = function(day, calInst)
        {
            var
                ...
                $day = $(ag.u.sprintf("<td class='day'>%s</td>", day.day))
                    .click(function(){
    ...
    calendarField.prototype.setValue = function(value)
    {
        ...
        if (this.minDay && this.selDay.compare(this.minDay) < 0)
            this.selDay = this.minDay;
        else if (this.maxDay && this.selDay.compare(this.maxDay) > 0)
    ...
    calendarField.prototype.setMinDay = function(day)
    {
        ...
        if (day && this.selDay.compare(day) <= 0)
        {
            this.selDay = day;
            this.month = this.selDay.month;
            this.year = this.selDay.year;
    ...
    calendarField.prototype.setMaxDay = function(day)
    {
        ...
        if (day && this.selDay.compare(this.maxDay) >= 0)
        {
            this.selDay = this.maxDay;
            this.month = this.selDay.month;
            this.year = this.selDay.year;
