...
(function(){
    var view = function(blocks)
    {
        ...
        this.body = this.find("section");
        this.footer = this.find("footer");

        Object.keys(blocks).forEach(function(name){
            var block = blocks[name];

            if (block instanceof ag.ui.ctxt.Block)
                blocks[name].setView(self);
            else
                window.console.log("block " + name + " must inherit from ag.ui.ctxt.Block!");
        });
    };
    ...
    {
        var titleBlock = this.getBlock(ag.ui.elem.Title);
    ...
    view.prototype.attachBlocks = function()
    {
        ...
        Object.keys(this.blocks).forEach(function(name){
            ...
            target.append(block);
            block.setView && block.setView(self);
        });
    };
    ...
    view.prototype.setStatus = function(status)
    {
        ...
        status && this.attachBlocks(); // attach again, in case some of them are shared with other views
    };
